...
BUILD_APK() {
    ...
    java -jar "${DOWNLOAD_DIR}/apktool.jar" b -f "${DOWNLOAD_DIR}/DECODE_Output" -o "${DOWNLOAD_DIR}/${GAME_BUNDLE_ID}.apk"
    if [ $? -ne 0 ]; then
        echo "错误: APK 构建失败！"
        exit 1
    fi
...
OPTIMIZE_AND_SIGN_APK() {
    ...
    local UNSIGNED_APK="${GAME_BUNDLE_ID}.unsigned.apk"
    local INPUT_APK="${DOWNLOAD_DIR}/${GAME_BUNDLE_ID}.apk"
...
# 获取并传回游戏版本
GET_GAME_VERSION() {
    if [ -f "${DOWNLOAD_DIR}/${GAME_BUNDLE_ID}.apk" ]; then
        if [ -f "$AAPT_PATH" ]; then
            GAME_VERSION=$("$AAPT_PATH" dump badging "${DOWNLOAD_DIR}/${GAME_BUNDLE_ID}.apk" | grep "versionName" | sed "s/.*versionName='\([^']*\)'.*/\1/" | head -1)
        ...
        echo "错误：APK文件不存在: ${DOWNLOAD_DIR}/${GAME_BUNDLE_ID}.apk"
    fi
...
# 重新打包XAPK
REPACK() {
    zip -r "${DOWNLOAD_DIR}/${GAME_BUNDLE_ID}.xapk" "${DOWNLOAD_DIR}/${GAME_BUNDLE_ID}"/*
}

# 生成7z分卷压缩包
CREATE_SPLIT_ARCHIVES() {
    local FINAL_APK="${DOWNLOAD_DIR}/${GAME_BUNDLE_ID}.xapk"
    if [ ! -f "$FINAL_APK" ]; then
